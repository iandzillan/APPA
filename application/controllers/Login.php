...
class Login extends CI_Controller {
    ...
    public function proses_user()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $cek    = $this->model_app->cek_user($u, $p);
            ...
            if ($cek->num_rows() > 0) {
                ...
                $session['akses']       = 'user';
                $this->session->set_userdata('penduduk', $session);
	...
	public function register_proses()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $cek_nik = $this->model_app->cek_register($nik)->num_rows();

            if ($cek_nik < 1) {
                $this->session->set_flashdata('error_nik','1');
                $this->load->view('login/register');
            } else {
                $this->model_app->register($nik);
